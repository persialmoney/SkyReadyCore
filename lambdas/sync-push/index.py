        ...
        for entry in changes.get('logbookEntries', {}).get('created', []):
            try:
                ...
                """, [
                    ...
                    entry.get('dayTouchAndGoLandings', 0),
                    entry.get('nightTakeoffs', 0), entry.get('nightLandings', 0),
                    entry.get('nightTouchAndGoLandings', 0),
                    entry.get('dayFullStopLandings', 0),
                    entry.get('nightFullStopLandings', 0),
                    entry.get('hobbsStart'), entry.get('hobbsEnd'),
                    entry.get('tachStart'), entry.get('tachEnd'),
                    entry.get('blockOut'), entry.get('blockIn'),
                    entry.get('onDuty'), entry.get('offDuty'),
                    entry.get('approaches', 0),
        ...
        for update in changes.get('logbookEntries', {}).get('updated', []):
            ...
            cursor.execute("""
                UPDATE logbook_entries SET
                    ...
                    simulated_instrument = %s,
                    day_takeoffs = %s, day_landings = %s, day_touch_and_go_landings = %s,
                    night_takeoffs = %s, night_landings = %s, night_touch_and_go_landings = %s,
                    day_full_stop_landings = %s, night_full_stop_landings = %s,
                    hobbs_start = %s, hobbs_end = %s, tach_start = %s, tach_end = %s,
                    block_out = %s, block_in = %s, on_duty = %s, off_duty = %s,
            ...
            """, [
                ...
                entry_data.get('dayTouchAndGoLandings', 0),
                entry_data.get('nightTakeoffs', 0), entry_data.get('nightLandings', 0),
                entry_data.get('nightTouchAndGoLandings', 0),
                entry_data.get('dayFullStopLandings', 0),
                entry_data.get('nightFullStopLandings', 0),
                entry_data.get('hobbsStart'), entry_data.get('hobbsEnd'),
                entry_data.get('tachStart'), entry_data.get('tachEnd'),
                entry_data.get('blockOut'), entry_data.get('blockIn'),
                entry_data.get('onDuty'), entry_data.get('offDuty'),
